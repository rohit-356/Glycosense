 ...
 *
 * In a Vite project  → set VITE_API_BASE_URL in .env
 * In a Next.js app   → set NEXT_PUBLIC_API_BASE_URL in .env.local
 * In Node/Jest tests → set API_BASE_URL in .env.test
 *
 * Never hardcode the URL here — that would break staging vs. production builds.
 */
const API_BASE_URL: string = (() => {
  // Try each env prefix in order; fall back to localhost only for local dev.
  const url =
    (typeof process !== 'undefined' && process.env?.VITE_API_BASE_URL) ||
    (typeof process !== 'undefined' && process.env?.NEXT_PUBLIC_API_BASE_URL) ||
    (typeof process !== 'undefined' && process.env?.API_BASE_URL) ||
    'http://127.0.0.1:8000'; // local dev default — NOT for production

  // Normalize: strip trailing slash so endpoint paths are always predictable
  return url.replace(/\/$/, '');
})();
...
// ---------------------------------------------------------------------------
// Public API: getRecentReadings (MOCK for Phase 1)
// ---------------------------------------------------------------------------

/**
 * Retrieves the most recent glucose readings for the user.
 * Connects to: GET /glucose/recent (Backend routing to be built later)
 *
 * Currently mocked to return sample data, allowing the React Native Frontend
 * (GlucoseScreen) to be developed before the database read layer is finalized.
 *
...
export async function getRecentReadings(
  ...
  // Validate basic inputs to mirror real endpoint behavior
  if (!userId) {
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve({
        success: true,
        data: [
          { id: 'mock-uuid-1', value: 110, timestamp: new Date(Date.now() - 1000 * 60 * 60 * 2).toISOString() }, // 2 hours ago
          { id: 'mock-uuid-2', value: 145, timestamp: new Date(Date.now() - 1000 * 60 * 60 * 8).toISOString() }, // 8 hours ago
          { id: 'mock-uuid-3', value: 95,  timestamp: new Date(Date.now() - 1000 * 60 * 60 * 24).toISOString() }, // 24 hours ago
        ].slice(0, limit),
      });
    }, 600);
  });
